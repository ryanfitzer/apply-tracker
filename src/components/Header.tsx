...
const Header = () => {
    ...
    return (<header className={`flex flex-col gap-3 pt-[10px] pl-[10px] sm:px-4 sm:py-2 sm:justify-between sm:flex-row  sm:w-full flex-shrink-0 ${uiItem.showMenu ? "w-[200px]" : "w-auto"}`}>
        <button className="sm:hidden text-3xl text-left"
            onClick={() => dispatch(uiActions.toggleMenu())}>{t("menu")}
        </button>
        <button className={`sm:hidden text-4xl text-center ${uiItem.showMenu ? "hidden" : "block"}`}
            onClick={() => dispatch(uiActions.toggleModal(true))}>+
        </button>
        <div className={`flex flex-col sm:flex-row sm:justify-between sm:w-full overflow-hidden ${uiItem.showMenu ? "w-full" : "w-0"}`}>
            <div className="flex flex-col sm:flex-row ">
                ...
                <div className="sm:ml-2 sm:border-l-2 sm:pl-2 flex">
                    <button onClick={
                        () => dispatch(uiActions.toggleChartsModal(true))}>{t("stats")}</button>
                </div>
            </div>
            <button className="text-left sm:text-center" onClick={clearAllJobs}>Clear All Jobs</button>
        </div>
